...
class UserProfile extends React.Component{ 
  constructor(props){
    super(props);
    console.log(this.props.user.user,'user iiiiiiiiiiiiiiiiiiiiiiiiiiiiiii');
    this.state = {
      showProfile: true,
      firstname : this.props.user ? this.props.user.user.firstname : "",
      lastname: this.props.user? this.props.user.user.lastname : "",
      username: this.props.user ? this.props.user.user.username : "",
      email: this.props.user ? this.props.user.user.email : "",
      dob: this.props.user ? this.props.user.user.dob : "",
      phonenumber: this.props.user ? this.props.user.user.phonenumber : "",
      photo: this.props.user ? this.props.user.user.photo : "",
      loading: false
    }
  }
  ...
  handleUpdate = async(e) => {
    ...
    axios.put(`/users/update/${this.props.user.user._id}`, this.state)
	  .then((res) => {
            ...
            <div style={{display: "grid", placeItems:"center", marginBottom: '70px'}}>
              <div style={{ width: "500px", margin:"20px auto", padding:'40px', boxShadow:" -0.5px -0.5px 0 0 rgba(0,0,0,0.175), 2px 2px 10px 1px rgba(0,0,0,0.175)" }}>
                ...
                <h5 style={{fontSize:'24px', textTransform:'uppercase', padding: "10px 0"}} className="userInfo">First Name :</h5>
                  <h3 style={{textTransform:"capitalize"}}>{this.state.firstname || "Not Available" }</h3>
                <h5 style={{fontSize:'24px', textTransform:'uppercase', padding: "10px 0"}} className="userInfo">Last Name :</h5>
                <h3 style={{textTransform:"capitalize"}}>{this.state.lastname || "Not Available"}</h3>
